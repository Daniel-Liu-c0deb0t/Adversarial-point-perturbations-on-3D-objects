...
@jit(nopython = True)
def _query(query_point, query_radius, curr_node, center, radius_lo, radius_hi, inside_node, outside_node, triangle, is_leaf):
    ...
    if is_leaf[curr_node]:
        if np.linalg.norm(query_point - center[curr_node]) <= query_radius:
            ...
            proj_point = project_point_to_triangle(query_point, triangle[curr_node], thickness = 0.0)
        ...
        if dist > radius_lo[curr_node] + query_radius: # query and partition spheres are completely not overlapping
            nearest = _query(query_point, query_radius, outside_node[curr_node], center, radius_lo, radius_hi, inside_node, outside_node, triangle, is_leaf)
        elif dist < radius_hi[curr_node] - query_radius: # query and partition spheres are completely overlapping
            nearest = _query(query_point, query_radius, inside_node[curr_node], center, radius_lo, radius_hi, inside_node, outside_node, triangle, is_leaf)
        else:
            # must examine both subtrees as the border of the query sphere overlaps the border of the partition sphere
            nearest_inside = _query(query_point, query_radius, inside_node[curr_node], center, radius_lo, radius_hi, inside_node, outside_node, triangle, is_leaf)
            nearest_outside = _query(query_point, query_radius, outside_node[curr_node], center, radius_lo, radius_hi, inside_node, outside_node, triangle, is_leaf)
...
@jit(nopython = True)
def _project(x_perturb, perturb, max_radius, root, center, radius_lo, radius_hi, inside_node, outside_node, triangle, is_leaf):
    epsilon = 1e-8
    ...
    for i in range(len(x_perturb)):
        if np.abs(distances[i]) < epsilon: # points that are not perturbed are also not projected
            ...
            nearest_point, nearest_dist = _query(x_perturb[i], distances[i] + max_radius, root, center, radius_lo, radius_hi, inside_node, outside_node, triangle, is_leaf)
...
@jit(nopython = True)
def _calc_tri_center(triangles):
    tri_center = []
    max_radius = 0.0
...
class TrueProjTree:
    ...
    def project(self, x_perturb, perturb):
        res = _project(x_perturb, perturb, self.max_radius, self.root, self.center, self.radius_lo, self.radius_hi, self.inside_node, self.outside_node, self.triangle, self.is_leaf)
